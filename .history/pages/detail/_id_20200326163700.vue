<template>
  <section>
    ...
    <div class="detail-content">
        ...
        <v-card class="detail">
          <div class="top d-flex justify-start align-start pa-4">
            ...
            <div class="detail__info">
              <h3 class="state pend">{{model.status}}</h3>
              <p class="hour">
                {{model.scheduled_date}} - {{model.scheduled_hour}}
              </p>
            </div>
          </div>
      ...
      <article>
        ...
        >
          ...
          <div class="contact__name">
            ...
            <p class="dni text-truncate">DNI {{contact.document}}</p>
          </div>
          <a
      ...
      <article class="actions d-flex justify-space-between align-center" v-if="model.status === 'Pendiente'">
        <v-btn
          class="decline"
          align="center"
          large
      ...
      <v-card class="dialog">
        ...
        <v-card-text class="pb-0">
          <v-radio-group v-model="declineMotive" column class="radioGroup">
            <v-radio label="No se encontro a nadie" :value="'No se encontro a nadie'"></v-radio>
            <v-radio label="Solicitó reprogramar" :value="'Solicitó reprogramar'"></v-radio>
            <v-radio label="Otro" :value="'Otro'"></v-radio>
          </v-radio-group>
          <v-expand-transition>
            <v-textarea
              v-show="declineMotive == 'Otro'"
              v-model="otherMotive"
              class="textArea"
              name="motivo"
              label="Motivo"
...
export default {
  ...
  async asyncData ({params, store}) {
    ...
    data.data.scheduled_date = $moment(data.data.scheduled_date).format('DD/MM/YY')
    const model = data.data

    const mapsUrl = `https://www.google.com/maps/place/${model.location.coordinates[0]}+${model.location.coordinates[1]}/@${model.location.coordinates[0]},${model.location.coordinates[1]},15z`
    const marker = model.location.coordinates;
    const center = model.location.coordinates;
    return {model, mapsUrl, marker, center}
  },
  mounted(){
    this.setVisitId(this.$route.params.id);
    this.setIsVisitData({isData: this.model.exists_data, literalCopy: this.model.literal_copy});
  },
  methods: {
    ...mapActions({
    ...
    async refuse() { 
      let motive = (this.declineMotive !== 'Otro')? this.declineMotive:this.otherMotive;
      $backend.declineVisit(this.$store.state.registerVisit.visitId, {motive})
