    ...
    <v-row>
      <div class="camera">
        <client-only>
          <WebCam
            ref="webcam"
            :aws-url="'https://api.dev.prestamype.com/v1/admin/visit/real_estate/upload_file/'+$store.state.registerVisit.visitId"
...
export default {
  ...
  computed: {
    ...mapState({
      lastPosition: state => state.registerVisit.lastPhotoPosition
    }),
    device() {
  ...
  mounted(){
    this.awsConfig = {position: this.lastPosition}
  },
  methods: {
    ...mapActions({
